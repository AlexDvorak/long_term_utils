...
fn main(){
    ...
    loop{
        ...
        let mut chunk: Vec<char> = buffer
            .iter()
            .map(|byte|
                 dec_to_ascii(*byte,&ascii_map))
            .collect();
        let mut found_eof: bool = false;
        for byte in chunk.iter(){ // DONT TOUCH THESE IF CONDITIONS
            if *byte == '`'{ //EOF code from dec_to_ascii
                println!("exited");
                found_eof = true;
                break;
            }
            ...
            if byte.to_owned() == '<' && (rec_tag_name == false){
                ...
                is_first_byte = true;
                rec_tag_data = false;
            }else if byte.to_owned() == '<' && rec_tag_data{ // detect start of tag name
                tag_data = tag_buff.clone().into_iter().collect();
                tag_buff = Vec::new();
                rec_tag_name = true;
                rec_tag_data = false;
            }else if rec_tag_name && byte.to_owned() == '>'{ // detect end of tag name
